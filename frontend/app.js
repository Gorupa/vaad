...
window.toggleView = function(viewName) {
    ...
    if (viewName === 'dashboard') {
        if (!currentUser) { alert("Please sign in to access your Practice Dashboard."); window.openModal(); return; }
...
window.handleSearch = async function() {
    ...
    if (activeJurisdiction === 'usa' && activeTab === 'us-case') {
        ...
        alert("Data-Driven Lawyer Discovery is currently compiling local records and will be available soon."); return;
...
function updateSearchLimitUI() {
    ...
    if (!currentUser) { if (limitText) limitText.innerText = "Sign in with Google to get 1 free search"; if (upgradeBtn) upgradeBtn.style.display = 'none'; return; }
    const fup = checkFUP('search');
    if (fup.expired) {
...
window.renderDashboard = function() {
    ...
    if (practiceCases.length === 0) {
        html = `<div style="text-align:center; padding: 40px 20px; color: var(--text-muted); border: 1px dashed var(--border); border-radius: 8px;">No cases tracked yet. Click "+ Add New Case" to start building your fee ledger.</div>`;
    }
    ...
    practiceCases.forEach(c => {
        ...
        <div id="dashboard-case-${c.id}" style="background: var(--bg); border: 1px solid var(--border); border-radius: 8px; margin-bottom: 16px; padding: 16px; transition: box-shadow 0.3s ease;">
            ...
            <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px; background: var(--bg-alt); padding: 12px; border-radius: 6px; margin-bottom: 12px; text-align: center;">
                <div><div style="font-size: 0.7rem; color: var(--text-muted); text-transform: uppercase;">Total Fee</div><div style="font-weight: 600;">₹${c.totalFee}</div></div>
                <div><div style="font-size: 0.7rem; color: var(--text-muted); text-transform: uppercase;">Per Hearing</div><div style="font-weight: 600;">₹${c.perHearing}</div></div>
                <div><div style="font-size: 0.7rem; color: var(--text-muted); text-transform: uppercase;">Collected</div><div style="font-weight: 600; color: var(--success-text);">₹${c.collected}</div></div>
            </div>

            <div style="display: flex; gap: 8px;">
                <input type="number" id="pay-input-${c.id}" placeholder="${c.perHearing > 0 ? 'e.g. ₹' + c.perHearing : '₹ Amount'}" style="flex: 1; padding: 8px; border: 1px solid var(--border); border-radius: 6px; font-size: 0.9rem;" ${remaining === 0 ? 'disabled' : ''}>
...
window.openAI = async function(legalText = null) {
    if (!currentUser) { alert("Please sign in to use AI."); return; }
